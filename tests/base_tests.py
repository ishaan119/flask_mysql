...
class BasicTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with app.test_request_context():
            ...
            # Adding seed data
            # Adding metadata
            meta_data = SolarSysMetadata(
                28, 10, 77, 'IN', 1, 1, 180, 28, 3, 'hourly')
            db.session.add(meta_data)
            # Adding reference data
            ref_data = ReferenceData(
                day=1, datapoints='(0,0,0,0,0,0,0,318.475,2489.115,4396.037,5645.406,6535.4,7011.977,6864.036,6429.069,4976.679,3135.839,885.004,0,0,0,0,0,0)', month=1, system_capacity=10)
            ...
            datapoints = [0, 0, 0, 0, 0, 0, 0, 180, 270, 600, 600,
                          630, 750, 930, 900, 1110, 1100, 810, 0, 0, 0, 0, 0, 0]
            for index, i in enumerate(datapoints):
                if index == 23:
                    date = date + datetime.timedelta(minutes=59)
                else:
                    date = date + datetime.timedelta(hours=1)
                datapoints_data = Datapoints(
                    dc_solar_power=i, date=date, system_identifier=3)
    ...
    def test_upload_solar_data(self):
        ...
        response = self.app.post('/apiv1/solar_data/3',
                                 data=dict(dc_solar_power=0,
                                           date='2015-01-21 01:00:00'),
